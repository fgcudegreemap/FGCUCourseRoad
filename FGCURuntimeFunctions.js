...
function updateViews() {
	if(hiddenOption.value=='Hide extra classes'){hidden=true;}else{hidden=false;}if(scienceOption.value=='Science Major'){science=true;}else{science=false;}
...
function displaySchedule() {
	...
	for (e=0;e<schedule.length;e++) {
		...
		element.setAttribute('class','classButton'+schedule[e].prereqStatus);
		element.id = schedule[e].number.replace(' ','_')+'_'+schedule[e].term+'_'+schedule[e].repeats;
		element.innerHTML = schedule[e].number;element.setAttribute("onclick","displayClassInfo(this.id);");
		//element.style.backgroundColor=schedule[e].prereqStatus;
		eval("term_"+schedule[e].term).appendChild(element);
	}
...
function GeneralEducationRequirements() {
	if (science) {
		try{
			generalEducationArray = ['General Education Program Requirements for a Science major<br>'];
		...
		try{
			generalEducationArray = ['General Education Program Requirements for a Non-Science major<br>'];
