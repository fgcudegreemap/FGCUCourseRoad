...
function updateViews() {
	...
	try{for (a=0;a<divs.length;a++) {if (divs[a].children.length == 1) {divs[a].firstChild.setAttribute("class","hiddenTerm");} else {divs[a].firstChild.setAttribute("class","visibleTerm");divs[a].firstChild.innerHTML = terms[j]+' - '+SumSemesterCredits(a)+' credit(s)';}}}catch(err){alert('Error setting up semesters for schedule display');}
...
function displaySchedule() {
	for (e=0;e<schedule.length;e++) {
		schedule[e].prereqStatus = prereqStatusF(schedule[e].number,schedule[e].term)[0];
		schedule[e].missingPrereqs = prereqStatusF(schedule[e].number,schedule[e].term)[1];
		element = document.getElementById(schedule[e].number.replace(' ','_')+'_'+schedule[e].term+'_'+schedule[e].repeats);
		element.style.backgroundColor=schedule[e].prereqStatus;
	}
	for (e=0;e<divs.length;e++) {if (divs[e].children.length == 1) {divs[e].firstChild.setAttribute("class","hiddenTerm");} else {divs[e].firstChild.setAttribute("class","visibleTerm");divs[e].firstChild.innerHTML = terms[e]+' - '+SumSemesterCredits(j)+' credit(s)';}}
}
// f
...
// k
function addClass(){
	classFound = false;classAddCheck = classAdd.value.toLowerCase();//alert(skipTo(classAddCheck));
	if (classAddCheck != classAdd.defaultValue.toLowerCase()) {for (k=skipTo(classAddCheck);k<Object.keys(classes).length;k++){if (classAddCheck == Object.keys(classes)[k].toLowerCase()) {classFound=true;schedule.push(new ClassInSchedule(semesterAdd.selectedIndex,Object.keys(classes)[k],classes[Object.keys(classes)[k]].name,classes[Object.keys(classes)[k]].credits,repeatsF(Object.keys(classes)[k]),classes[Object.keys(classes)[k]].description,prereqStatusF(Object.keys(classes)[k],semesterAdd.selectedIndex)[0],classes[Object.keys(classes)[k]].prereqs,prereqStatusF(Object.keys(classes)[k],semesterAdd.selectedIndex)[1]));element = document.createElement("button");element.setAttribute('class','specialButton');element.id = schedule[schedule.length-1].number.replace(' ','_')+'_'+schedule[schedule.length-1].term+'_'+schedule[schedule.length-1].repeats;element.innerHTML = schedule[schedule.length-1].number;element.setAttribute("onclick","displayClassInfo(this.id);");eval("term_"+semesterAdd.selectedIndex).appendChild(element);break;}}if (!classFound) { courseDescriptionParagraph.innerHTML = '<redText>Please be sure to enter a valid class to add.</redText>';}}
	else { courseDescriptionParagraph.innerHTML = '<redText>Please be sure to enter a valid class to add.</redText>';}
	displaySchedule();updateViews(); 
}
//l
function removeClass() {
	classFound = false;classRemoveCheck = classRemove.value.toLowerCase();//alert(skipTo(classRemoveCheck));
	if (classRemoveCheck != classRemove.defaultValue.toLowerCase()) {if (schedule.length === 0) {courseDescriptionParagraph.innerHTML = '<redText>You dont have any classes in your schedule yet to remove.</redText>';}else {for (l=0;l<schedule.length;l++) {if (classRemoveCheck == schedule[i].number.toLowerCase() && semesterRemove.selectedIndex == schedule[i].term) {classFound = true;eval("term_"+semesterRemove.selectedIndex).removeChild(document.getElementById(schedule[i].number.replace(' ','_')+'_'+schedule[i].term+'_'+schedule[i].repeats));schedule.splice(i,1);break;}}}}
	if (classFound) {for (l=0;l<schedule.length;l++) {if (schedule[l].number.toLowerCase() == classRemoveCheck && schedule[l].repeats > 0) {newRepeats = schedule[l].repeats-1;element = document.getElementById(schedule[l].number.replace(' ','_')+'_'+schedule[l].term+'_'+schedule[l].repeats);element.id = schedule[l].number.replace(' ','_')+'_'+schedule[l].term+'_'+newRepeats;schedule[l].repeats--;}}}else { courseDescriptionParagraph.innerHTML = '<redText>Please be sure to enter a valid class in your schedule and the semester to remove it from.</redText>';}
	displaySchedule();updateViews();
}
// m,n
