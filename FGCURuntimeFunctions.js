...
function removeClass() {
	classFound = false;classRemoveCheck = classRemove.value.toLowerCase();
	if (classRemoveCheck != classRemove.defaultValue.toLowerCase()) {if (schedule.length === 0) {courseDescriptionParagraph.innerHTML = '<redText>You dont have any classes in your schedule yet to remove.</redText>';}else {for (l=0;l<schedule.length;l++) {if (classRemoveCheck == schedule[l].number.toLowerCase() && semesterRemove.selectedIndex == schedule[l].term) {classFound = true;eval("term_"+semesterRemove.selectedIndex).removeChild(document.getElementById(schedule[l].number.replace(' ','_')+'_'+schedule[l].term+'_'+schedule[l].repeats));schedule.splice(i,1);courseDescriptionParagraph.innerHTML="";break;}}}}
